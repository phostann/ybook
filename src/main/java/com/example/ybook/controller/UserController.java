...
public class UserController {
    ...
    public ApiResult<List<UserVO>> listUsers() {
        return ApiResult.success(userService.listAllUsers());
    }
    ...
    public ApiResult<PageResult<UserVO>> pageUsers(
            @Parameter(description = "当前页码", example = "1")
            @RequestParam(defaultValue = "1") long current,
            @Parameter(description = "每页数量", example = "10")
            @RequestParam(defaultValue = "10") long size) {
        Page<UserEntity> page = new Page<>(current, size);
        return ApiResult.success(userService.pageUsers(page));
    }
    ...
    public ApiResult<UserVO> updateUser(@PathVariable Long id, @Valid @RequestBody UserUpdateDTO userUpdateDTO) {
        return ApiResult.success(userService.updateUser(id, userUpdateDTO));
    }
